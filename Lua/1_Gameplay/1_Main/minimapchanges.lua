...
addHook("PostThinkFrame", function()
	...
	if displayplayers[0].mo and displayplayers[0].mo.valid and 
		displayplayers[0].mo.timetravel and displayplayers[0].mo.timetravel.isTimeWarped then
		if minimaplib.getMinimapPatchString() ~= minimapReverseName then
			minimaplib.setMinimapPatchByString(minimapReverseName)
		end
	else
		if minimaplib.getMinimapPatchString() ~= minimapName then
...
local function headPositionModify(v, mo, moX, moY, flags, scale, patch, colormap)
	...
	if leveltime % TICRATE <= 2 then
		...
		if P1 and P1.mo and P1.mo.valid and P1.mo ~= mo then
			local status = P1.mo.timetravel.isTimeWarped
